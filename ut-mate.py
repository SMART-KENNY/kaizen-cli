...
def log_and_report(job, step, message):
    print(message)
    wb = openpyxl.load_workbook(report_path)
    ws = wb.active
    ws.append([job, step, message])
    wb.save(report_path)
...
def check_table_column_comments(table_name, tags_file_path="GDM_Databricks_Tag_List.csv"):
    ...
    for _, tag_row in df_csv.iterrows():
        ...
        # print(f"field_names are : {field_names}")
        tag_status = ""
        tag_command = ""
        for row in columns:
...
if __name__ == "__main__":
    ...
    for job in filtered_jobs:
        for file_path in glob.glob(f"{etc_path}*{job}*"):
                ...
                check_table_column_comments(table_name=dev_table_name, tags_file_path=tags_file_path)